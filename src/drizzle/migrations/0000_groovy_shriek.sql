...
CREATE TYPE "public"."job_listing_type" AS ENUM('internship', 'full-time', 'part-time', 'contract');--> statement-breakpoint
CREATE TYPE "public"."job_listing_location_requirement" AS ENUM('remote', 'onsite', 'hybrid');--> statement-breakpoint
CREATE TYPE "public"."job_listing_wage_interval" AS ENUM('hourly', 'daily', 'weekly', 'monthly', 'yearly');--> statement-breakpoint
CREATE TYPE "public"."job_listing_application_stage" AS ENUM('applied', 'interested', 'interviewed', 'hired', 'rejected');--> statement-breakpoint
CREATE TABLE "users" (
...
CREATE TABLE "organization_user_settings" (
	...
	CONSTRAINT "organization_user_settings_email_unique" UNIQUE("email")
);
--> statement-breakpoint
